  ...
  const handleSubmit = async () => {
    ...
    if (currentTodo.value.tags) {
        ...
        tags.forEach((tag) => {
            if (tag) { // Asegurarse de que el tag no esté vacío
    ...
    try {
        if (isEditing.value) {
            await api.put(`/notes/${currentTodo.value.id}`, formData);
  ...
  const formatDate = (dateString) => {
    ...
    return isNaN(date.getTime()) ? 'Fecha no válida' : date.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' });
  }
